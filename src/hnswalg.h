...
namespace hnswlib {
    ...
    class HierarchicalNSW : public AlgorithmInterface<dist_t> {
        ...
        std::priority_queue<std::pair<dist_t, labeltype >>
            searchBaseLayerST(std::vector<unsigned> eps, const void* data_point, size_t ef) const {
            VisitedList* vl = visited_list_pool_->getFreeVisitedList();
            vl_type* visited_array = vl->mass;
            vl_type visited_array_tag = vl->curV;
        ...
        template <bool has_deletions, bool collect_metrics = false>
        std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst>
            searchBaseLayerSTWithEp(tableint ep_id, const void* data_point, size_t ef) const {
            VisitedList* vl = visited_list_pool_->getFreeVisitedList();
            vl_type* visited_array = vl->mass;
            vl_type visited_array_tag = vl->curV;

            auto& distfunc = fstdistfunc_;
        ...
        void loadIndex(const std::string& location, SpaceInterface<dist_t>* s, size_t max_elements_i = 0) {
            ...
            std::cout << "Loading HNSW from " << location << ":\n";
        ...
        std::priority_queue<std::pair<dist_t, labeltype >>
            searchKnnInternalLabel(const void* query_data, size_t k) const {
            std::priority_queue<std::pair<dist_t, labeltype >> result;
            if(cur_element_count == 0) return result;
        ...
        std::unordered_map<int, int> count_degree() const {
            std::vector<int> res(max_elements_, 0);
            for(int i = 0; i < cur_element_count; i++) {
                tableint current_node_id = i;
                int* data = (int*)get_linklist0(current_node_id);
                size_t size = getListCount((linklistsizeint*)data);
                tableint* datal = (tableint*)(data + 1);
            ...
            std::unordered_map<int, int> mp_in_deg;// Convert internel id to external id
            for(int i = 0; i < cur_element_count; i++) {
                mp_in_deg[getExternalLabel(i)] = res[i];
            }
            return mp_in_deg;
        }
